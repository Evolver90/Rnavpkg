...
methods via particularly efficient inner products, though at the time of writing these
inner products are only at the bit level.
...
necessarily a goal, but we do want to learn or verify characteristics of particular algorithms,
which includes their workload.
...
related to numerical methods infrastructure in, or related to, R:

- package lbfgsb3: the 2011 Fortran code for L-BFGS-B (@Morales:2011)
- RQmin: a vignette about Rayleigh Quotient minimization
 (\url{https://r-forge.r-project.org/scm/viewvc.php/RQdoc/RQtimes.pdf?root=optimizer&view=log})
- snewton: a developmental package for a safeguarded Newton method (@Nash-snewton)
- pracma: a collection of numerical methods (@Borchers-pracma)
- geradin: an implementation of a special Rayleigh Quotient minimizer (JN?)
- Nash-Beleites and others: a vignette about various packages in R for PLS and singular
value decomposition computations (\url{https://gitlab.com/nashjc/svdpls})
- jacobi: Bill Venables recently approached me about the original Jacobi eigenvalue method
(\url{https://cran.r-project.org/web/packages/JacobiEigen/vignettes/JacobiEigen.pdf}),
derived from the 19th century paper @Jacobi1846
- several PORT library (@Fox-PORT1978) routines are used in R, notably in nlminb() for bound 
constrained function minimization and nonlinear least squares (in nls())
...
- what does R already use in base
- what does R already use or offer in packages
- other codes
